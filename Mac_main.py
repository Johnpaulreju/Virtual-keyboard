...
# Define the virtual keyboard layout with additional keys (updated for Mac)
keys = [
    ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'BACKSPACE'],
    ['TAB', 'Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P'],
    ['CAPS', 'A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'ENTER'],
    ['SHIFT', 'Z', 'X', 'C', 'V', 'B', 'N', 'M', '<', '>', 'SPACE'],
    ['CMD', 'ALT', 'COPY', 'PASTE', 'YOUTUBE', 'GOOGLE', 'CURSOR']
]
...
# State variables
modifier_states = {'SHIFT': False, 'CMD': False, 'ALT': False, 'CAPS': False}
...
last_pinch_time = 0
pinch_threshold = 0.08  # Increased threshold for pinch detection
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if hover_key is not None and pinch_distance < pinch_threshold:
                if current_time - last_pinch_time >= pinch_cooldown:
                    ...
                    if key in ['SHIFT', 'CMD', 'ALT']:
                        modifier_states[key] = not modifier_states[key]
                    elif key == 'CAPS':
                        modifier_states['CAPS'] = not modifier_states['CAPS']
                    elif key == 'COPY':
                        pyautogui.hotkey('command', 'c')  # Updated for Mac
                    elif key == 'PASTE':
                        pyautogui.hotkey('command', 'v')  # Updated for Mac
                    elif key == 'YOUTUBE':
                        ...
                        if modifier_states['CMD']:
                            pyautogui.hotkey('command', key.lower())  # Updated for Mac
